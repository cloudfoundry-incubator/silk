...
func (g *DeviceNameGenerator) generate(prefix, suffix string, containerIP net.IP) (string, error) {
	i := containerIP.To4()
	if i == nil {
		return "", errors.New("generating device name: expecting valid IPv4 address")
	}
	return fmt.Sprintf("%s-%03d%03d%03d%03d%s", prefix, i[0], i[1], i[2], i[3], suffix), nil
}
...
func (g *DeviceNameGenerator) GenerateForHostIFB(containerIP net.IP) (string, error) {
	return g.generate("s", "-ifb", containerIP)
}
